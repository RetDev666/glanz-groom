...
# 1. toggleService -> toggleService (multi-pet)
text = re.sub(
    r"const toggleService = \(id: number, isPackage: boolean\) => \{[\s\S]*?\}\);\s+\};",
...
# 2. handlePhotoChange
text = re.sub(
    r"const handlePhotoChange = async \(e: React\.ChangeEvent<HTMLInputElement>\) => \{[\s\S]*?\}\);\s+\};",
    """const handlePhotoChange = async (petIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        setError(t.book.step3.photoError || 'Photo error');
        return;
      }
...
# 3. Replace Step 0 UI block completely
# Find {step === 0 && (  ... )}
step_0_regex = r"\{\/\* ── STEP 0: Services ── \*\/\}[\s\S]*?\{\/\* ── STEP 1: Groomer ── \*\/\}   "
        ...
        {step === 0 && (
          <div className="flex flex-col gap-6">
            ...
            <button onClick={addPet} className="w-full py-4 border-2 border-dashed border-primary text-primary rounded-2xl font-sans text-label-lg hover:bg-primary-container transition-all flex items-center justify-center gap-2">
              ...
              Додати ще собаку (Add another dog)
            </button>
          </div>
        )}
...
text = re.sub(step_0_regex, new_step_0, text)


# 4. Replace Step 3 UI block (Details)
step_3_regex = r"\{\/\* ── STEP 3: Details ── \*\/\}[\s\S]*?\{\/\* BOTTOM ACTION BAR \*\/\}   "
                  ...
                  <div className="mt-4">
                    ...
                    >
                      <input 
                        ...
                        className="hidden" 
                        accept="image/*" 
                        onChange={(e) => handlePhotoChange(petIndex, e)}
                      />
                      {pet.photoPreview ? (
...
text = re.sub(step_3_regex, new_step_3, text)


# 5. Fix success page
# Find <strong>{booking.petName}</strong> and change it to show all pet names
text = text.replace(
    "<strong>{booking.petName}</strong>",
    "<strong>{booking.pets.map(p => p.petName).join(', ')}</strong>"
)

# And the success photo preview? It used `booking.photoPreview`. We'll just hide it or show the first one.
text = text.replace(
