  ...
  return (
    <footer className="w-full py-12 px-6 md:px-12 border-t border-surface-variant bg-surface mt-auto">
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 border-t border-surface-variant/60">
          <nav className="flex flex-wrap justify-center gap-4 sm:gap-6">
            {legalLinks.map(link => (
              <Link
                key={link.href}
                href={link.href}
                className="text-xs font-sans text-on-surface-variant hover:text-primary transition-colors"
              >
                {link.label}
              </Link>
            ))}
